...
func validate(ctx context.Context, a *template.TemplateDtoV2) url.Values {
	errs := url.Values{}
	...
	// Check if Text actually has content
	// HTML, Attachments and Embedded (Attachments) can be empty
	if len(a.Text) < 1 {
		errs.Add("template", "Text cannot be empty")
	}
