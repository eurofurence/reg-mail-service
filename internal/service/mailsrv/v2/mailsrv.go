...
func (s *MailServiceImplData) SendMail(ctx context.Context, dto mail.MailSendDto, template entity.TemplateV2, textBody string, htmlBody string) error {
	...
	// Attachments
	for _, attachment := range template.Attachments {
		m.Attach()
	}
	...
	if !config.MailLogOnly() {
		...
		logTargets(ctx, dto)
		aulogging.Logger.Ctx(ctx).Info().Printf("Subject: %s", template.Subject)
		aulogging.Logger.Ctx(ctx).Info().Printf(textBody)
		aulogging.Logger.Ctx(ctx).Info().Printf(htmlBody)
	}
